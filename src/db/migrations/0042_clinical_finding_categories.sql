-- Migration 0042 — Align clinical_findings with the categorized finding model.
-- The API writes category, anatomical_site, and JSON detail columns.
...
SELECT
  ...
  END AS category,
  CASE
    WHEN scope = 'tooth' THEN 'tooth'
    WHEN scope IN ('full_mouth', 'occlusion') THEN 'full_mouth'
    ELSE 'region'
  END AS scope,
  tooth_number,
  tooth_system,
