...
router.get(
  ...
  async (c) => {
    ...
    const items = await appointmentsService.list(c.env.DB, jwt.tenant_id, {
      ...
      status: (url.searchParams.get("status") as "scheduled" | "confirmed" | "completed" | "cancelled" | "no_show" | null) ?? undefined,
    });
...
router.post(
  ...
  async (c) => {
    ...
    const created = await appointmentsService.create(
      c.env.DB, jwt.tenant_id, jwt.sub,
      { ...data, branch_id: jwt.branch_id },
      encKey,
    );
