...
router.post("/rewards/:id/mark-paid", requirePermission(PERMISSIONS.PAY_REFERRAL_REWARDS), blockDemoAction("Đánh dấu đã chi thưởng"), auditLog("mark_paid", "referral_reward"), zValidator("json", referralCashPaidSchema), async (c) => { const jwt = getJwt(c); const data = c.req.valid("json"); return c.json(await referralService.markCashPaid(c.env.DB, jwt.tenant_id, jwt.sub, c.req.param("id"), data.payment_method, data.payment_reference)); });
router.post("/rewards/:id/issue-voucher", requirePermission(PERMISSIONS.REVIEW_REFERRAL_REWARDS), blockDemoAction("Phát hành voucher"), auditLog("issue_voucher", "referral_reward"), async (c) => { const jwt = getJwt(c); return c.json(await referralService.issueVoucher(c.env.DB, jwt.tenant_id, jwt.sub, c.req.param("id"))); });
