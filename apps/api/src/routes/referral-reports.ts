...
router.get("/", async (c) => {
  ...
  const summary = await c.env.DB.prepare(
    `SELECT COUNT(*) AS case_count,
     ...
     FROM referral_cases c LEFT JOIN referral_rewards rw ON rw.referral_case_id = c.id WHERE c.tenant_id = ?`,
  ).bind(jwt.tenant_id).first();
  const byReferrer = await c.env.DB.prepare(
    `SELECT r.id, r.name, r.code, r.type, COUNT(c.id) AS case_count, COALESCE(SUM(rw.calculated_amount), 0) AS reward_total
     ...
     WHERE r.tenant_id = ? GROUP BY r.id ORDER BY case_count DESC, reward_total DESC LIMIT 100`,
  ).bind(jwt.tenant_id).all();
  ...
  return c.json({
    ...
    items: byReferrer.results.map((row) => ({
      ...
      case_status: "pending_conversion",
      registered_at: "",
