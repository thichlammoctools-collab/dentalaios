...
import type { PlatformJwtPayload } from "@shared/types";
import {
  PLATFORM_MFA_STEP_UP_SECONDS,
  PLATFORM_SESSION_IDLE_SECONDS,
} from "@shared/constants";
import type { Env } from "../index";
import { verifyPlatformJwt } from "../lib/platform-jwt";
...
export function getPlatformJwt(c: {
  get(key: "platformJwt"): PlatformJwtPayload | undefined;
}): PlatformJwtPayload {
  const jwt = c.get("platformJwt");
  if (!jwt) throw new Error("Platform JWT not set");
  return jwt;
}
export function requireRecentPlatformMfa(): MiddlewareHandler<{
  Bindings: Env;
  Variables: PlatformAuthContext;
}> {
  return async (c, next) => {
    const at = c.get("platformMfaVerifiedAt");
    if (!at || !valid(at, PLATFORM_MFA_STEP_UP_SECONDS))
      throw new ForbiddenError("Recent MFA verification required");
    await next();
  };
}
