...
export function ManagementDashboardPage() {
  ...
  const outcomeRate = snapshot?.kpis && snapshot.kpis.appointments > 0
    ? ((snapshot.kpis.cancellations + snapshot.kpis.no_shows) / snapshot.kpis.appointments) * 100
    : undefined;
  ...
  return <PageContainer size="workspace" className="space-y-5 sm:space-y-6">
    ...
    {error ? <Card><CardHeader><CardTitle>Không thể tải tổng quan</CardTitle><CardDescription>{error}</CardDescription></CardHeader><CardContent><Button onClick={() => void loadSnapshot(true)}>Thử lại</Button></CardContent></Card> : loading ? <DashboardSkeleton /> : noActivity ? <EmptyDashboard /> : <>
      ...
      <section><div className="mb-3"><h2 className="text-lg font-semibold">Kết quả {range} ngày đã hoàn tất</h2><p className="text-sm text-muted-foreground">So sánh cùng số ngày ngay trước kỳ được chọn.</p></div><div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6"><MetricCard primary label="Doanh thu xác nhận" value={formatCurrency(numberFrom(snapshot?.kpis, "confirmed_revenue", "revenue") ?? 0)} delta={percentChange(numberFrom(snapshot?.kpis, "confirmed_revenue", "revenue"), numberFrom(snapshot?.kpis, "previous_revenue"))} money /><MetricCard primary label="Lượt khám" value={formatNumber(numberFrom(snapshot?.kpis, "visits"))} delta={percentChange(numberFrom(snapshot?.kpis, "visits"), numberFrom(snapshot?.kpis, "previous_visits"))} /><MetricCard label="Tỷ lệ hoàn thành" value={formatPercent(numberFrom(snapshot?.kpis, "completion_rate"))} /><MetricCard label="Bệnh nhân mới" value={formatNumber(numberFrom(snapshot?.kpis, "new_patients"))} /><MetricCard label="Lịch hẹn" value={formatNumber(numberFrom(snapshot?.kpis, "appointments"))} /><MetricCard label="Kế hoạch chờ duyệt" value={formatNumber(numberFrom(snapshot?.kpis, "pending_plans", "pending_plan_count"))} alert={Boolean(numberFrom(snapshot?.kpis, "pending_plans", "pending_plan_count"))} /></div></section>
      <section><div className="mb-3 flex items-baseline justify-between gap-3"><div><h2 className="text-lg font-semibold">Rủi ro cần theo dõi</h2><p className="text-sm text-muted-foreground">Ưu tiên xử lý hủy/vắng và kế hoạch tồn trước khi ảnh hưởng kết quả kỳ.</p></div><span className="text-xs text-muted-foreground">{outcomeRate === undefined ? "Chưa có lịch hẹn trong kỳ" : `${new Intl.NumberFormat("vi-VN", { maximumFractionDigits: 1 }).format(outcomeRate)}% lịch hẹn hủy/vắng`}</span></div><div className="grid grid-cols-2 gap-3 md:grid-cols-3"><MetricCard label="Hủy + không đến" value={formatNumber((numberFrom(snapshot?.kpis, "cancellations") ?? 0) + (numberFrom(snapshot?.kpis, "no_shows") ?? 0))} alert /><MetricCard label="Đã hủy" value={formatNumber(numberFrom(snapshot?.kpis, "cancellations"))} alert /><MetricCard label="Không đến" value={formatNumber(numberFrom(snapshot?.kpis, "no_shows"))} alert /></div></section>
...
function AttentionList({ items, range, onSelect }: { items: AttentionItem[]; range: DashboardRange; onSelect: (id: string) => void }) { const priority: Record<string, number> = { overdue_appointment: 0, appointment_outcome: 1, pending_plan: 2 }; const sorted = [...items].sort((left, right) => (priority[left.kind] ?? 9) - (priority[right.kind] ?? 9) || right.count - left.count); return <Card><CardHeader><CardTitle>Cần chú ý</CardTitle><CardDescription>Ưu tiên theo mức độ ảnh hưởng vận hành; chỉ hiển thị số lượng tổng hợp theo chi nhánh.</CardDescription></CardHeader><CardContent>{sorted.length === 0 ? <p className="py-6 text-center text-sm text-muted-foreground">Không có mục nào cần theo dõi.</p> : <ul className="space-y-3">{sorted.map((item, index) => { const id = getBranchId(item); const isSchedule = /overdue|appointment_outcome|cancel|no_show/.test(item.kind); const urgent = item.kind === "overdue_appointment"; return <li className={`rounded-lg border p-3 ${urgent ? "border-red-200 bg-red-50/50 dark:border-red-900 dark:bg-red-950/20" : "border-border"}`} key={`${item.kind}-${id}-${index}`}><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-medium">{attentionLabel(item.kind)}</p><p className="mt-0.5 text-xs text-muted-foreground">{getBranchName(item)} · {urgent ? "Cần xử lý ngay" : "Cần theo dõi"}</p></div><span className={`rounded-full px-2 py-0.5 text-sm font-semibold ${urgent ? "bg-red-100 text-red-900 dark:bg-red-950 dark:text-red-100" : "bg-amber-100 text-amber-900 dark:bg-amber-950 dark:text-amber-100"}`}>{formatNumber(item.count)}</span></div><div className="mt-2 flex gap-3 text-xs"><button type="button" onClick={() => onSelect(id)} className="text-primary hover:underline">Xem tổng quan</button>{isSchedule ? <Link className="text-primary hover:underline" to={`/schedule?branch_id=${encodeURIComponent(id)}`}>Xem lịch hẹn</Link> : <Link className="text-primary hover:underline" to={`${dashboardRoute}?range=${range}&branch_id=${encodeURIComponent(id)}`}>Giữ ngữ cảnh</Link>}</div></li>; })}</ul>}</CardContent></Card>; }
