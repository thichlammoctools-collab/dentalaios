...
interface BranchSummary { id?: string; branch_id?: string; name?: string; branch_name?: string }
interface DailyPoint { date: string; visits?: number; revenue?: number; confirmed_revenue?: number }
interface BranchPerformance extends BranchSummary {
  revenue?: number; confirmed_revenue?: number; revenue_delta?: number | null; visits?: number;
  visits_delta?: number | null; appointments?: number; completion_rate?: number | null;
  new_patients?: number; pending_plans?: number; pending_plan_count?: number;
  cancellations?: number; cancellation_count?: number; no_shows?: number; no_show_count?: number;
}
interface AttentionItem extends BranchSummary { kind: string; count: number }
interface DashboardSnapshot {
  metadata?: { generated_at?: string; today_start?: string; current_day_start?: string; available_branches?: BranchSummary[] };
  generated_at?: string; available_branches?: BranchSummary[]; today?: Record<string, number | null | undefined>;
  range?: Record<string, number | null | undefined>; daily?: DailyPoint[]; daily_series?: DailyPoint[];
  series?: DailyPoint[]; branch_comparison?: BranchPerformance[]; branch_performance?: BranchPerformance[];
  exceptions?: AttentionItem[]; attention_items?: AttentionItem[];
}

const dashboardRoute = (ROUTES as Record<string, string>).MANAGEMENT_DASHBOARD ?? "/management-dashboard";
const dashboardPermission = "view_management_dashboard";
...
function getBranchId(branch: BranchSummary) { return branch.branch_id ?? branch.id ?? ""; }
function getBranchName(branch: BranchSummary) { return branch.branch_name ?? branch.name ?? "Chi nhánh chưa đặt tên"; }
...
export function ManagementDashboardPage() {
  ...
  const hasPermission = Boolean(session?.role.permissions.includes("all") || session?.role.permissions.includes(dashboardPermission));
  ...
  async function loadSnapshot(manual = false) {
    ...
    try {
      ...
      setSnapshot(next);
      setLastUpdated(next.metadata?.generated_at ?? next.generated_at ?? new Date().toISOString());
    } catch (cause) {
      if (currentRequest !== requestId.current) return;
  ...
  const branches = snapshot?.metadata?.available_branches ?? snapshot?.available_branches ?? [];
  const daily = snapshot?.daily ?? snapshot?.daily_series ?? snapshot?.series ?? [];
  const comparisons = snapshot?.branch_comparison ?? snapshot?.branch_performance ?? [];
  const attention = snapshot?.attention_items ?? snapshot?.exceptions ?? [];
  ...
  return <div className="mx-auto max-w-7xl space-y-5 p-4 sm:space-y-6 sm:p-6">
    <section className="overflow-hidden rounded-2xl bg-gradient-to-br from-slate-950 via-blue-950 to-indigo-900 p-5 text-white shadow-lg sm:p-7">
      <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end"><div><p className="text-sm font-medium text-blue-200">{session?.tenant.name}</p><h2 className="mt-1 text-2xl font-semibold tracking-tight sm:text-3xl">Quản trị tổng quan</h2><p className="mt-2 text-sm text-blue-100">Vận hành toàn hệ thống · {hcmDate(snapshot?.metadata?.today_start ?? snapshot?.metadata?.current_day_start)}</p></div>
    ...
    {error ? <Card><CardHeader><CardTitle>Không thể tải tổng quan</CardTitle><CardDescription>{error}</CardDescription></CardHeader><CardContent><Button onClick={() => void loadSnapshot(true)}>Thử lại</Button></CardContent></Card> : loading ? <DashboardSkeleton /> : noActivity ? <EmptyDashboard /> : <>
      ...
      <section><div className="mb-3"><h2 className="text-lg font-semibold">Hiệu quả {range} ngày đã hoàn tất</h2><p className="text-sm text-muted-foreground">So sánh cùng số ngày ngay trước kỳ được chọn.</p></div><div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6"><MetricCard label="Doanh thu xác nhận" value={formatCurrency(numberFrom(snapshot?.range, "confirmed_revenue", "revenue") ?? 0)} delta={numberFrom(snapshot?.range, "revenue_delta", "confirmed_revenue_delta")} money /><MetricCard label="Lượt khám" value={formatNumber(numberFrom(snapshot?.range, "visits"))} delta={numberFrom(snapshot?.range, "visits_delta")} /><MetricCard label="Lịch hẹn" value={formatNumber(numberFrom(snapshot?.range, "appointments"))} /><MetricCard label="Tỷ lệ hoàn thành" value={formatPercent(numberFrom(snapshot?.range, "completion_rate"))} /><MetricCard label="Bệnh nhân mới" value={formatNumber(numberFrom(snapshot?.range, "new_patients"))} /><MetricCard label="Kế hoạch chờ duyệt" value={formatNumber(numberFrom(snapshot?.range, "pending_plans", "pending_plan_count"))} /></div></section>
...
function AttentionList({ items, range, onSelect }: { items: AttentionItem[]; range: DashboardRange; onSelect: (id: string) => void }) { return <Card><CardHeader><CardTitle>Cần chú ý</CardTitle><CardDescription>Chỉ hiển thị số lượng tổng hợp theo chi nhánh.</CardDescription></CardHeader><CardContent>{items.length === 0 ? <p className="py-6 text-center text-sm text-muted-foreground">Không có mục nào cần theo dõi.</p> : <ul className="space-y-3">{items.map((item, index) => { const id = getBranchId(item); const isSchedule = /cancel|no_show|overdue/.test(item.kind); return <li className="rounded-lg border border-border p-3" key={`${item.kind}-${id}-${index}`}><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-medium">{attentionLabel(item.kind)}</p><p className="mt-0.5 text-xs text-muted-foreground">{getBranchName(item)}</p></div><span className="rounded-full bg-amber-100 px-2 py-0.5 text-sm font-semibold text-amber-900 dark:bg-amber-950 dark:text-amber-100">{formatNumber(item.count)}</span></div><div className="mt-2 flex gap-3 text-xs"><button type="button" onClick={() => onSelect(id)} className="text-primary hover:underline">Xem tổng quan</button>{isSchedule ? <Link className="text-primary hover:underline" to={`/schedule?branch_id=${encodeURIComponent(id)}`}>Xem lịch hẹn</Link> : <Link className="text-primary hover:underline" to={`${dashboardRoute}?range=${range}&branch_id=${encodeURIComponent(id)}`}>Giữ ngữ cảnh</Link>}</div></li>; })}</ul>}</CardContent></Card>; }
function BranchComparison({ rows, onSelect }: { rows: BranchPerformance[]; onSelect: (id: string) => void }) { return <Card><CardHeader><CardTitle>So sánh chi nhánh</CardTitle><CardDescription>Chọn một chi nhánh để lọc toàn bộ tổng quan.</CardDescription></CardHeader><CardContent>{rows.length === 0 ? <p className="py-8 text-center text-sm text-muted-foreground">Chưa có chi nhánh hoặc dữ liệu so sánh.</p> : <><div className="space-y-3 md:hidden">{rows.map((row) => <button type="button" key={getBranchId(row)} onClick={() => onSelect(getBranchId(row))} className="w-full rounded-lg border border-border p-4 text-left hover:bg-accent"><div className="flex justify-between gap-4"><span className="font-medium">{getBranchName(row)}</span><span className="font-semibold">{formatCurrency(row.confirmed_revenue ?? row.revenue ?? 0)}</span></div><div className="mt-3 grid grid-cols-3 gap-2 text-xs text-muted-foreground"><span>{formatNumber(row.visits)} khám</span><span>{formatNumber(row.appointments)} lịch</span><span>{formatPercent(row.completion_rate)} hoàn thành</span><span>{formatNumber(row.new_patients)} BN mới</span><span>{formatNumber(row.pending_plans ?? row.pending_plan_count)} chờ duyệt</span><span>{formatNumber((row.cancellations ?? row.cancellation_count ?? 0) + (row.no_shows ?? row.no_show_count ?? 0))} hủy/vắng</span></div></button>)}</div><div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[900px] text-sm"><thead className="border-b text-left text-xs text-muted-foreground"><tr><th className="p-3">Chi nhánh</th><th className="p-3 text-right">Doanh thu</th><th className="p-3 text-right">Lượt khám</th><th className="p-3 text-right">Lịch hẹn</th><th className="p-3 text-right">Hoàn thành</th><th className="p-3 text-right">BN mới</th><th className="p-3 text-right">Chờ duyệt</th><th className="p-3 text-right">Hủy/vắng</th></tr></thead><tbody>{rows.map((row) => <tr key={getBranchId(row)} className="border-b last:border-0 hover:bg-accent/50"><td className="p-3"><button type="button" onClick={() => onSelect(getBranchId(row))} className="font-medium text-primary hover:underline">{getBranchName(row)}</button></td><td className="p-3 text-right">{formatCurrency(row.confirmed_revenue ?? row.revenue ?? 0)}</td><td className="p-3 text-right">{formatNumber(row.visits)}</td><td className="p-3 text-right">{formatNumber(row.appointments)}</td><td className="p-3 text-right">{formatPercent(row.completion_rate)}</td><td className="p-3 text-right">{formatNumber(row.new_patients)}</td><td className="p-3 text-right">{formatNumber(row.pending_plans ?? row.pending_plan_count)}</td><td className="p-3 text-right">{formatNumber((row.cancellations ?? row.cancellation_count ?? 0) + (row.no_shows ?? row.no_show_count ?? 0))}</td></tr>)}</tbody></table></div></>}</CardContent></Card>; }
