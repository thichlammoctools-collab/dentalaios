...
function Home({ t, href }: { t: SiteContent; href: Href }) {
  return <>
    <section className="marketing-hero relative overflow-hidden border-b border-[#dce4de]"><div className="marketing-dental-grid absolute inset-0" aria-hidden="true" /><div className="relative z-10 mx-auto max-w-7xl px-5 pb-20 pt-16 lg:px-8 lg:pb-28 lg:pt-24"><div className="grid items-center gap-12 lg:grid-cols-[1fr_.95fr] lg:gap-20"><div><p className="motion-enter motion-enter-stagger mb-5 flex items-center gap-2 text-xs font-bold uppercase tracking-[0.16em] text-[#367461]" style={{ "--motion-delay": "0ms" } as React.CSSProperties}><Sparkles size={15} />{t.heroEyebrow}</p><h1 className="motion-enter motion-enter-stagger max-w-2xl text-4xl font-semibold leading-[1.08] tracking-[-0.045em] text-[#123c31] sm:text-5xl lg:text-[4.25rem]" style={{ "--motion-delay": "60ms" } as React.CSSProperties}>{t.heroTitle}</h1><p className="motion-enter motion-enter-stagger mt-6 max-w-xl text-lg leading-8 text-[#587068]" style={{ "--motion-delay": "120ms" } as React.CSSProperties}>{t.heroBody}</p><div className="motion-enter motion-enter-stagger mt-9 flex flex-wrap gap-3" style={{ "--motion-delay": "180ms" } as React.CSSProperties}><Link to={href("contact")} className="inline-flex items-center gap-2 rounded-xl bg-[#123c31] px-5 py-3.5 font-semibold text-white shadow-lg shadow-[#123c31]/15">{t.start}<ArrowRight size={17} /></Link><Link to={href("features")} className="inline-flex items-center gap-2 rounded-xl border border-[#cfddd2] bg-white px-5 py-3.5 font-semibold text-[#245044]">{t.seeFeatures}</Link></div></div><div className="motion-enter motion-enter-stagger relative" style={{ "--motion-delay": "240ms" } as React.CSSProperties}><div className="marketing-preview-glow absolute left-1/2 top-1/2 size-[82%] -translate-x-1/2 -translate-y-1/2 rounded-full" aria-hidden="true" /><ClinicPreview t={t} /></div></div></div></section>
...
function Cta({ t, href }: { t: SiteContent; href: Href }) { return <section className="mx-auto max-w-7xl px-5 pb-20 pt-20 lg:px-8 lg:pb-28 lg:pt-28"><div className="rounded-3xl bg-[#dceee0] px-7 py-12 text-center sm:px-12"><h2 className="mx-auto max-w-2xl text-3xl font-semibold leading-tight tracking-[-0.035em] text-[#123c31] sm:text-4xl">{t.ctaTitle}</h2><p className="mx-auto mt-4 max-w-xl leading-7 text-[#547268]">{t.ctaBody}</p><p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-[#547268]">{t.demo === "Trải nghiệm demo" ? "Demo dành cho chủ phòng khám: toàn quyền trên dữ liệu mô phỏng, có audit và chặn các thao tác gửi ra bên ngoài." : "The owner demo provides full access to simulated data, with auditing and external actions safely blocked."}</p><div className="mt-7 flex flex-wrap justify-center gap-3"><Link to="/login?demo=owner" className="marketing-demo-cta inline-flex items-center gap-2 rounded-xl bg-[#123c31] px-5 py-3.5 text-sm font-semibold text-white">{t.demo}<ArrowRight className="marketing-demo-cta-icon" size={16} /></Link><Link to={href("contact")} className="inline-flex items-center gap-2 rounded-xl border border-[#7da58c] px-5 py-3.5 text-sm font-semibold text-[#123c31]">{t.demo === "Trải nghiệm demo" ? "Đặt lịch tư vấn" : "Book a consultation"}</Link></div></div></section>; }
