...
export function PlatformClinicalTerminologyPage() {
  ...
  const [importJson, setImportJson] = useState("[\n  {\n    \"code\": \"K02.9\",\n    \"display_vi\": \"Sâu răng, không xác định\",\n    \"is_billable\": true\n  }\n]");
  ...
  async function importCodes(event: FormEvent) {
    event.preventDefault();
    try {
      const codesInput = JSON.parse(importJson) as Array<{ code: string; display_vi: string; parent_code?: string; is_billable?: boolean; sort_order?: number }>;
    ...
    } catch (cause) { setError(cause instanceof SyntaxError ? "JSON mã ICD-10 không hợp lệ" : cause instanceof PlatformApiError ? cause.message : "Không thể import ICD-10"); }
  }
  ...
  return <div className="mx-auto max-w-7xl space-y-6 p-4 sm:p-7">
    ...
    <Dialog open={importOpen} onOpenChange={setImportOpen}><form onSubmit={importCodes}><DialogHeader><DialogTitle>Import ICD-10 Việt Nam</DialogTitle></DialogHeader><DialogBody className="space-y-3"><Label htmlFor="import-version">Phiên bản nháp</Label><Select id="import-version" value={importVersionId} onChange={(event) => setImportVersionId(event.target.value)} required><option value="">Chọn phiên bản</option>{icdDrafts.map((version) => <option key={version.id} value={version.id}>{version.title}</option>)}</Select><Label htmlFor="import-json">Mảng JSON từ artifact đã xác minh</Label><Textarea id="import-json" className="min-h-60 font-mono text-xs" value={importJson} onChange={(event) => setImportJson(event.target.value)} required /></DialogBody><DialogFooter><Button type="button" variant="outline" onClick={() => setImportOpen(false)}>Hủy</Button><Button type="submit">Import</Button></DialogFooter></form></Dialog>
