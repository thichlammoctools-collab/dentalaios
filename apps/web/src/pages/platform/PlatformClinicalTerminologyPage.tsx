...
export function PlatformClinicalTerminologyPage() {
  ...
  async function createMapping(event: FormEvent) {
    ...
    catch (cause) { setError(cause instanceof PlatformApiError ? cause.message : "Không thể lưu ánh xạ"); }
  }
  ...
  return <div className="mx-auto w-full max-w-[90rem] space-y-6 p-4 sm:p-7 lg:px-8 lg:py-8 2xl:px-10">
    <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#67e8f9]">Quản trị nền tảng</p><h1 className="mt-1 text-2xl font-semibold">Thuật ngữ lâm sàng</h1><p className="mt-1 max-w-3xl text-sm text-muted-foreground">Danh mục khái niệm nội bộ, ánh xạ và phiên bản ICD-10 Việt Nam có nguồn gốc rõ ràng.</p></div>{canWrite && <div className="flex flex-wrap gap-2"><Button onClick={() => setQuickSetupOpen(true)}>Thiết lập ICD-10 nhanh</Button><Button variant="outline" onClick={() => setMappingOpen(true)}>Tạo ánh xạ</Button><Button variant="outline" onClick={() => { setImportVersionId(icdDrafts[0]?.id ?? ""); setImportOpen(true); }}>Nhập ICD-10</Button><Button variant="outline" onClick={() => setConceptOpen(true)}>Thêm khái niệm</Button><Button variant="outline" onClick={() => setVersionOpen(true)}>Thêm phiên bản</Button></div>}</div>
    ...
    <Dialog open={mappingOpen} onOpenChange={setMappingOpen}><form onSubmit={createMapping}><DialogHeader><DialogTitle>Tạo ánh xạ khái niệm - ICD-10</DialogTitle></DialogHeader><DialogBody className="space-y-3"><Select value={mappingForm.concept_id} onChange={(event) => setMappingForm({ ...mappingForm, concept_id: event.target.value })} required><option value="">Chọn khái niệm</option>{concepts.filter((concept) => concept.kind === "diagnosis").map((concept) => <option key={concept.id} value={concept.id}>{concept.display_vi}</option>)}</Select><Select value={mappingForm.icd10_code_id} onChange={(event) => setMappingForm({ ...mappingForm, icd10_code_id: event.target.value })} required><option value="">Chọn ICD-10</option>{codes.map((code) => <option key={code.id} value={code.id}>{code.code} · {code.display_vi}</option>)}</Select><Select value={mappingForm.mapping_role} onChange={(event) => setMappingForm({ ...mappingForm, mapping_role: event.target.value })}><option value="primary">Chính</option><option value="alternative">Thay thế</option></Select></DialogBody><DialogFooter><Button type="button" variant="outline" onClick={() => setMappingOpen(false)}>Hủy</Button><Button type="submit">Lưu ánh xạ</Button></DialogFooter></form></Dialog>
  </div>;
}
