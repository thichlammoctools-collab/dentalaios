...
export function PlatformClinicalTerminologyPage() {
  ...
  const [mappingForm, setMappingForm] = useState({ concept_id: "", icd10_code_id: "" });
  ...
  async function importCodes(event: FormEvent) {
    ...
    } catch (cause) { setError(cause instanceof PlatformApiError ? cause.message : cause instanceof Error ? cause.message : "Không thể nhập ICD-10"); }
  }
  async function setupIcd10(event: FormEvent) {
    event.preventDefault();
    try {
      const version = await platformPost<TerminologyVersion>("/api/platform/clinical-terminology/versions", versionForm);
      await platformPost("/api/platform/clinical-terminology/icd10/import", { terminology_version_id: version.id, codes: parseIcd10Import(importJson) });
      await platformPatch(`/api/platform/clinical-terminology/versions/${version.id}/status`, { status: "approved" });
      setQuickSetupOpen(false);
      load();
    } catch (cause) { setError(cause instanceof PlatformApiError ? cause.message : cause instanceof Error ? cause.message : "Không thể thiết lập ICD-10"); }
  }
  async function createMapping(event: FormEvent) {
    ...
    catch (cause) { setError(cause instanceof PlatformApiError ? cause.message : "Không thể gắn ICD-10 chính"); }
  }
  ...
  return <div className="mx-auto w-full max-w-[90rem] space-y-6 p-4 sm:p-7 lg:px-8 lg:py-8 2xl:px-10">
    <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#67e8f9]">Quản trị nền tảng</p><h1 className="mt-1 text-2xl font-semibold">Thuật ngữ lâm sàng</h1><p className="mt-1 max-w-3xl text-sm text-muted-foreground">Danh mục chẩn đoán dùng một ICD-10 chính, có nguồn gốc và phiên bản rõ ràng.</p></div>{canWrite && <div className="flex flex-wrap gap-2"><Button onClick={() => setQuickSetupOpen(true)}>Thiết lập ICD-10 nhanh</Button><Button variant="outline" onClick={() => setMappingOpen(true)}>Gắn ICD-10 chính</Button><Button variant="outline" onClick={() => { setImportVersionId(icdDrafts[0]?.id ?? ""); setImportOpen(true); }}>Nhập ICD-10</Button><Button variant="outline" onClick={() => setConceptOpen(true)}>Thêm khái niệm</Button><Button variant="outline" onClick={() => setVersionOpen(true)}>Thêm phiên bản</Button></div>}</div>
    ...
    <Card><CardHeader><CardTitle>Phiên bản và nguồn</CardTitle><CardDescription>ICD-10 chỉ được approve sau khi nhập từ artifact chính thức có publisher, file nguồn và SHA-256.</CardDescription></CardHeader><CardContent className="overflow-x-auto p-0"><table className="w-full min-w-[760px] text-sm"><thead className="border-y text-left text-muted-foreground"><tr><th className="p-3">Hệ mã</th><th className="p-3">Phiên bản</th><th className="p-3">Nguồn</th><th className="p-3">Trạng thái</th><th className="p-3" /></tr></thead><tbody>{versions.map((version) => <tr className="border-b" key={version.id}><td className="p-3 font-mono text-xs">{version.system}</td><td className="p-3"><p className="font-medium">{version.title}</p><p className="text-xs text-muted-foreground">{version.version_key}</p></td><td className="p-3 text-xs">{version.publisher ?? "--"}<br />{version.source_file_name ?? "--"}</td><td className="p-3"><span className="rounded-full bg-muted px-2 py-1 text-xs">{version.status}</span></td><td className="p-3 text-right">{canWrite && version.status === "draft" && <Button size="sm" variant="outline" onClick={() => void approve(version)}>Duyệt</Button>}</td></tr>)}{versions.length === 0 && <tr><td className="p-8 text-center text-muted-foreground" colSpan={5}>Chưa có phiên bản.</td></tr>}</tbody></table></CardContent></Card>
     <div className="grid gap-6 lg:grid-cols-2"><Card><CardHeader><CardTitle>Danh mục chẩn đoán</CardTitle><CardDescription>Mỗi chẩn đoán dùng một mã ICD-10 chính do quản trị viên gắn.</CardDescription></CardHeader><CardContent className="max-h-[32rem] space-y-2 overflow-auto">{concepts.filter((concept) => concept.kind === "diagnosis").map((concept) => <div className="rounded-lg border p-3 text-sm" key={concept.id}><div className="flex items-start justify-between gap-3"><div><p className="font-medium">{concept.display_vi}</p><p className="mt-1 font-mono text-xs text-muted-foreground">{concept.code}</p></div><span className={`rounded-full px-2 py-1 text-xs ${concept.default_icd10 ? "bg-emerald-500/10 text-emerald-600" : "bg-amber-500/10 text-amber-600"}`}>{concept.default_icd10 ? concept.default_icd10.code : "Chưa gắn mã"}</span></div><p className="mt-2 text-xs text-muted-foreground">{concept.default_icd10?.display_vi ?? "Bác sĩ chỉ có thể lưu nghi ngờ cho đến khi được gắn ICD-10."}</p></div>)}{concepts.filter((concept) => concept.kind === "diagnosis").length === 0 && <p className="text-sm text-muted-foreground">Chưa có chẩn đoán trong danh mục.</p>}</CardContent></Card><Card><CardHeader><CardTitle>Mã ICD-10 đã duyệt</CardTitle></CardHeader><CardContent className="max-h-[32rem] space-y-2 overflow-auto">{codes.map((code) => <div className="rounded-lg border p-3 text-sm" key={code.id}><p className="font-mono text-xs font-semibold">{code.code}</p><p className="mt-1">{code.display_vi}</p></div>)}{codes.length === 0 && <p className="text-sm text-muted-foreground">Chưa có mã ICD-10 approved. Tạo version, import artifact và duyệt trước.</p>}</CardContent></Card></div>
    ...
    <Dialog open={quickSetupOpen} onOpenChange={setQuickSetupOpen}><form onSubmit={setupIcd10}><DialogHeader><DialogTitle>Thiết lập ICD-10 nhanh</DialogTitle></DialogHeader><DialogBody className="space-y-3"><p className="text-sm text-muted-foreground">Hoàn tất tạo phiên bản, import bảng mã và duyệt trong một lần. Chỉ dùng artifact ICD-10 đã xác minh.</p><Input placeholder="Mã phiên bản, ví dụ: icd10-vn-2026" value={versionForm.version_key} onChange={(event) => setVersionForm({ ...versionForm, system: "ICD10_VN", version_key: event.target.value })} required /><Input placeholder="Tên phiên bản" value={versionForm.title} onChange={(event) => setVersionForm({ ...versionForm, system: "ICD10_VN", title: event.target.value })} required /><Input placeholder="Cơ quan ban hành" value={versionForm.publisher} onChange={(event) => setVersionForm({ ...versionForm, system: "ICD10_VN", publisher: event.target.value })} required /><Input placeholder="Tên tệp nguồn" value={versionForm.source_file_name} onChange={(event) => setVersionForm({ ...versionForm, system: "ICD10_VN", source_file_name: event.target.value })} required /><Input placeholder="SHA-256 (64 hex)" value={versionForm.source_sha256} onChange={(event) => setVersionForm({ ...versionForm, system: "ICD10_VN", source_sha256: event.target.value })} required /><Input placeholder="URL nguồn (tùy chọn)" value={versionForm.source_url} onChange={(event) => setVersionForm({ ...versionForm, system: "ICD10_VN", source_url: event.target.value })} /><Label htmlFor="quick-import">Bảng mã ICD-10</Label><Textarea id="quick-import" className="min-h-48 font-mono text-xs" value={importJson} onChange={(event) => setImportJson(event.target.value)} required /></DialogBody><DialogFooter><Button type="button" variant="outline" onClick={() => setQuickSetupOpen(false)}>Hủy</Button><Button type="submit">Thiết lập và duyệt</Button></DialogFooter></form></Dialog>
    ...
    <Dialog open={importOpen} onOpenChange={setImportOpen}><form onSubmit={importCodes}><DialogHeader><DialogTitle>Import ICD-10 Việt Nam</DialogTitle></DialogHeader><DialogBody className="space-y-3"><Label htmlFor="import-version">Phiên bản nháp</Label><Select id="import-version" value={importVersionId} onChange={(event) => setImportVersionId(event.target.value)} required><option value="">Chọn phiên bản</option>{icdDrafts.map((version) => <option key={version.id} value={version.id}>{version.title}</option>)}</Select><Label htmlFor="import-json">Bảng mã từ artifact đã xác minh</Label><p className="text-xs text-muted-foreground">Dán bảng hai cột (mã và mô tả), có hoặc không có dấu chấm trong mã. JSON vẫn được hỗ trợ.</p><Textarea id="import-json" className="min-h-60 font-mono text-xs" value={importJson} onChange={(event) => setImportJson(event.target.value)} required /></DialogBody><DialogFooter><Button type="button" variant="outline" onClick={() => setImportOpen(false)}>Hủy</Button><Button type="submit">Import</Button></DialogFooter></form></Dialog>
     <Dialog open={mappingOpen} onOpenChange={setMappingOpen}><form onSubmit={createMapping}><DialogHeader><DialogTitle>Gắn ICD-10 chính</DialogTitle></DialogHeader><DialogBody className="space-y-3"><p className="text-sm text-muted-foreground">Mã mới sẽ thay thế mã primary hiện tại của chẩn đoán. Hồ sơ chẩn đoán đã lưu vẫn giữ snapshot cũ.</p><Select value={mappingForm.concept_id} onChange={(event) => setMappingForm({ ...mappingForm, concept_id: event.target.value })} required><option value="">Chọn chẩn đoán</option>{concepts.filter((concept) => concept.kind === "diagnosis").map((concept) => <option key={concept.id} value={concept.id}>{concept.display_vi}</option>)}</Select><Select value={mappingForm.icd10_code_id} onChange={(event) => setMappingForm({ ...mappingForm, icd10_code_id: event.target.value })} required><option value="">Chọn ICD-10 chính</option>{codes.map((code) => <option key={code.id} value={code.id}>{code.code} · {code.display_vi}</option>)}</Select></DialogBody><DialogFooter><Button type="button" variant="outline" onClick={() => setMappingOpen(false)}>Hủy</Button><Button type="submit">Gắn mã chính</Button></DialogFooter></form></Dialog>
  </div>;
}
