...
export function ReferrersPage() {
  ...
  return <PageContainer size="data">
    ...
    <div className="overflow-x-auto rounded-xl border border-border bg-card shadow-sm">
      {!items.length ? <ReferralEmpty>Chưa có Người giới thiệu.</ReferralEmpty> : <table className="w-full min-w-[920px] text-sm"><thead className="border-b bg-muted/30 text-left text-xs text-muted-foreground"><tr><th className="p-3 font-medium">Người giới thiệu</th><th className="p-3 font-medium">Mã</th><th className="p-3 font-medium">Liên hệ</th><th className="p-3 font-medium">Trạng thái</th><th className="p-3 font-medium">Tạo lúc</th>{canManage && <th className="p-3" />}</tr></thead><tbody className="divide-y">{items.map((item) => <tr key={item.id}><td className="p-3"><p className="font-medium">{item.name}</p><p className="text-xs text-muted-foreground"><ReferrerTypeLabel type={item.type} /></p></td><td className="p-3"><code className="rounded bg-muted px-2 py-1 text-xs font-semibold">{item.code}</code></td><td className="p-3 text-muted-foreground">{item.email ?? item.phone ?? "-"}</td><td className="p-3"><ReferrerStatusBadge status={item.status} /></td><td className="p-3 text-xs text-muted-foreground">{formatDateTime(item.created_at)}</td>{canManage && <td className="p-3 text-right"><div className="flex justify-end gap-2"><Button size="sm" variant="outline" onClick={() => openEdit(item)}>Sửa</Button><Button size="sm" variant="outline" onClick={() => void regenerateCode(item)}>Cấp lại mã</Button><Button size="sm" variant="outline" onClick={() => { setAccountFor(item); setAccountEmail(item.email ?? ""); }}>Portal</Button><Button size="sm" variant="ghost" onClick={() => void setStatus(item)}>{item.status === "active" ? "Ngừng" : "Bật"}</Button></div></td>}</tr>)}</tbody></table>}
    </div>
    {items.length > 0 && <details className="rounded-xl border border-border bg-card p-4 shadow-sm"><summary className="cursor-pointer font-medium">Mã QR Người giới thiệu</summary><p className="mt-2 text-sm text-muted-foreground">Mỗi QR chỉ chứa ID nội bộ của Người giới thiệu. Khi quét, hệ thống kiểm tra referrer thuộc phòng khám hiện tại và còn hoạt động trước khi tự chọn.</p><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{items.map((item) => <ReferrerQrCode key={item.id} referrerId={item.id} label={`${item.name} (${item.code})`} />)}</div></details>}
