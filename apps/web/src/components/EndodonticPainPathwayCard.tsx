...
export function EndodonticPainPathwayCard({ visitId, canWrite, canReview }: EndodonticPainPathwayCardProps) {
  ...
  if (!data?.feature_enabled) {
    return <Card className="border-border/50 border-dashed bg-muted/20" id="clinical-copilot">
      <CardContent className="p-6 text-center">
        <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-cyan-500/10 mb-3">
        ...
        <p className="font-semibold">Clinical Copilot chưa được kích hoạt</p>
        <p className="mt-1 text-sm text-muted-foreground">Phòng khám cần bật feature flag <code className="text-xs bg-muted px-1 rounded">clinical_copilot.endodontic_pain_v1</code> trong cài đặt nền tảng để sử dụng tính năng đánh giá đau răng/nội nha.</p>
      </CardContent>
    </Card>;
  }
